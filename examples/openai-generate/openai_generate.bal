...
public function main() returns error? {
    string blogContent = string `
        Ballerina is a cloud-native programming language designed for
        integration. It has built-in support for network protocols like
        HTTP, gRPC, and WebSockets. The language treats network interactions
        as first-class citizens, making it ideal for building microservices
        and APIs. Ballerina also features a visual diagram representation
        of the code, making it easier to understand data flows.
    `;
    ...
    BlogMetadata metadata = check model->generate(
        ...
        Provide a suitable title, 3-5 relevant tags, and a
