...
public function main() returns error? {
    ...
    ai:QueryMatch[] queryMatches =
        check knowledgeBase.retrieve(appealQuery, 10);
    ...
    string answer = check modelProvider->generate(
    ...
    // The `augmentUserQuery` function augments the user query with the
    // context using a generic prompt template.
    ai:ChatUserMessage augmentedQuery =
        ai:augmentUserQuery(context, carryForwardQuery);

    // Use the `chat` method with the augmented query.
