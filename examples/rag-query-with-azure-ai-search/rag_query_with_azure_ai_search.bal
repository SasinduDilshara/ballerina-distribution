...
final azure:EmbeddingProvider embeddingProvider = check new (
    azureOpenAiEmbeddingUrl, azureOpenAiEmbeddingToken,
...
final azure:OpenAiModelProvider chatModel = check new (
    azureOpenAiServiceUrl, azureOpenAiApiKey,
    chatDeploymentId, azureOpenAiApiVersion
);
...
public function main() returns error? {
    ...
    ai:ChatAssistantMessage answer1 =
        check chatModel->chat(augmentedQuery1, []);
    ...
    ai:ChatAssistantMessage answer2 =
        check chatModel->chat(augmentedQuery2, []);
    io:println("\nQuery: ", query2);
