...
This example demonstrates how to use the default embedding provider to embed a document and a batch of candidate texts, and compare the similarity of each candidate with the document. To use embeddings in a knowledge base for RAG, see the [Retrieve from an in-memory vector store](/learn/by-example/rag-in-memory-vector-store-retrieval/) example. To use a specific provider with your own keys, see the [Generate embeddings with a specific provider](/learn/by-example/rag-embedding-provider/) example.
